...
if [ ! -f configs/tinymaix.json ] ; then
    echo "ERROR: Please run script in tinymaxi folder, not in the configs folder or other places!"
    echo "INFO: eg. ./configs/run_qemu.sh"
    exit 1
fi
...
echo "INFO: Start to run tinymaxi examples on qemu for rv32 and rv64 cores!"
runcmd="python3 ${NSDK_BENCH_CLI} --appcfg configs/tinymaix.json --hwcfg configs/qemu.json --logdir ${LOGDIR} --run_target qemu --run --parallel=\"-j\""
